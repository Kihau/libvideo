...
public class YouTube
{
    ...
    public async Task<IEnumerable<YouTubeVideo>> GetAllVideosAsync(string videoUri)
    {
        if (!TryNormalize(videoUri, out videoUri))
            throw new ArgumentException("URL is not a valid YouTube URL!");

        var client = new HttpClient();
        string source = await client.GetStringAsync(videoUri);

        return ParseVideos(source);
    }

    public IEnumerable<YouTubeVideo> GetAllVideos(string videoUri)
        => this.GetAllVideosAsync(videoUri).GetAwaiter().GetResult();

    public async Task<YouTubeVideo> GetVideoAsync(string videoUri)
    {
        var video = await this.GetAllVideosAsync(videoUri);
        return video.First();
    }
    ...
    private IEnumerable<YouTubeVideo> ParseVideos(string source)
    {
        ...
        var parsed = ParsePlayerJson(source);
        var extracted = Json.Extract(parsed);
        var playerResponseJson = JToken.Parse(extracted);
        ...
        else // player_response
        {
            ...
            foreach (var item in streamObjects)
            {
                ...
                var cipherValue =
                    ((item.SelectToken("cipher") ?? item.SelectToken("signatureCipher")) ?? string.Empty)
                    .Value<string>();
    ...
    private string ParsePlayerJson(string source)
    {
        ...
        if ((match = Regex.Match(source, ytPlayerPattern)).Success &&
            Json.TryGetKey("player_response", match.Groups[1].Value, out string json))
        ...
        if (string.IsNullOrWhiteSpace(playerResponseMap) &&
            (match = Regex.Match(source, ytInitialPlayerPattern)).Success)
            playerResponseMap = match.Groups[1].Value;
            
        string ytWindowInitialPlayerResponse = @"\[\""ytInitialPlayerResponse\""\]\s*=\s*(\{.*\});";
        if (string.IsNullOrWhiteSpace(playerResponseMap) &&
            (match = Regex.Match(source, ytWindowInitialPlayerResponse)).Success)
            playerResponseMap = match.Groups[1].Value;
    ...
    {
        if (Json.TryGetKey("jsUrl", source, out var jsPlayer) ||
            Json.TryGetKey("PLAYER_JS_URL", source, out jsPlayer))
        {
        ...
        else
        {
            ...
            Match match = Regex.Match(source,
                "<script\\s*src=\"([-a-zA-Z0-9()@:%_\\+.~#?&//=]*)\".*name=\"player_ias/base\".*>\\s*</script>");
    ...
    {
        int start = manifestUri.IndexOf(playback, StringComparison.Ordinal) + playback.Length;
    ...
    public static string GetSignatureKey() =>
        string.IsNullOrWhiteSpace(_signatureKey) ? "signature" : _signatureKey;
}